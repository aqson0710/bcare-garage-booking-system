...
type LoadState =
  ...
  | { status: "signed-out"; error: null; notifications: null; userId: null }
  | {
      status: "ready";
      error: null;
  ...
  | { status: "error"; error: string; notifications: null; userId: null };
...
function getReadStorageKey(userId: string) {
  return `bcare-read-notifications:${userId}`;
}
...
function writeStoredNotificationIds(userId: string, readIds: Set<string>) {
  window.localStorage.setItem(
    ...
    JSON.stringify([...readIds].slice(-200)),
  );
}
...
export function CustomerNotificationsPanel() {
  ...
  useEffect(() => {
    ...
    async function loadNotifications() {
      ...
      setLoadState({
        error: null,
        notifications: buildCustomerNotifications({
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--muted)]">
              รวมสถานะสำคัญของการจอง งานซ่อม คำสั่งซื้อสินค้า และการชำระเงิน
            </p>
          </div>
        ...
        <section className="py-6">
          ...
          {visibleNotifications.length > 0 ? (
            ...
            <div className="rounded-lg border border-dashed border-[var(--line)] bg-white p-6 text-sm leading-6 text-[var(--muted)]">
              ...
              <p className="mt-1">
                เมื่อมีการยืนยันการจอง อัปเดตงานซ่อม เปลี่ยนสถานะออเดอร์
                หรือผลตรวจชำระเงิน รายการจะมาแสดงที่นี่
              </p>
            </div>
          )}
        </section>
